...
public class LoadAsyncTask extends AsyncTask<String, Void, String> {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            URL url = new URL(Configuration.URL + "api/v1/loads/" + username);
        ...
        try {
            ...
            if(!jsonObject.isNull("data")) {
                if(context.getClass().getSimpleName().equals("LoadPointActivity")) {
                    ((LoadPointActivity) context).loadText.setText(
                            jsonObject.getJSONObject("data").getString("Amount"));
                } else {
                    ((HomeActivity) context).loadText.setText(
                            jsonObject.getJSONObject("data").getString("Amount"));
                }
            } else {
                if(context.getClass().getSimpleName().equals("LoadPointActivity")) {
                    ((LoadPointActivity) context).loadText.setText("0");
                } else {
                    ((HomeActivity) context).loadText.setText("0");
                }
            }
